//! 与 provider 无关的 agent loop，从 Onemore 原样移植。
//!
//! loop 只负责编排模型调用；有状态的行为全部通过 [`AgentLoopHost`] 的回调接入。
//! IF 的 `IfTaskHost`（docs/05 §3）实现这些回调：Jev 审查提议、完成条件判定、停止与修复指令。
...
use model_call::{call_model, ModelCallResult};
