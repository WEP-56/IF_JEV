...
export default function Sidebar(p: Props) {
  ...
  const [blankMenuAt, setBlankMenuAt] = useState<{ x: number; y: number } | null>(null);
  ...
  return (
    <aside
      className="flex h-full w-[260px] shrink-0 flex-col bg-chrome"
      onContextMenu={(event) => {
        if ((event.target as HTMLElement).closest('[data-story-row]')) return;
    ...
    >
      ...
      <div className="mt-4 flex-1 overflow-y-auto px-2 pb-2" onClick={() => { setMenu(null); setMenuAt(null); }}>
                ...
                return (
                    ...
                    {menu === s.id && (
                      <div
                        ...
                        style={menuAt ? { position: 'fixed', left: menuAt.x, top: menuAt.y } : undefined}
                        className={cn('fade-up z-50 w-40 rounded-xl border border-line bg-elev p-1 text-[13px] shadow-2xl shadow-black/30', menuAt ? 'fixed' : 'absolute top-9 right-0')}
                      >
                        {[
                          ...
                          <button
                            key={it.label}
                            onClick={() => { it.fn(); setMenu(null); setMenuAt(null); }}
                        ...
                        <button
                          onClick={() => { p.onDelete(s.id); setMenu(null); setMenuAt(null); }}
      ...
      {blankMenuAt && (
        <div
          ...
          onMouseDown={() => setBlankMenuAt(null)}
          onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); }}
        >
          <div
            role="menu"
            style={{ left: blankMenuAt.x, top: blankMenuAt.y }}
            onMouseDown={(event) => event.stopPropagation()}
            className="fade-up fixed z-[60] min-w-40 rounded-lg border border-line bg-elev p-1 text-[13px] shadow-xl"
          >
            <button role="menuitem" onClick={() => { p.onNew(); setBlankMenuAt(null); }} className="flex w-full items-center rounded-md px-2.5 py-1.5 text-left hover:bg-subtle">
              <SquarePen size={14} className="mr-2 text-muted" /> 新建故事
            </button>
            <button role="menuitem" onClick={() => { setSearching(true); setTimeout(() => searchRef.current?.focus(), 0); setBlankMenuAt(null); }} className="flex w-full items-center rounded-md px-2.5 py-1.5 text-left hover:bg-subtle">
              <Search size={14} className="mr-2 text-muted" /> 搜索故事
            </button>
            <button role="menuitem" onClick={() => { p.onSettings(); setBlankMenuAt(null); }} className="flex w-full items-center rounded-md px-2.5 py-1.5 text-left hover:bg-subtle">
              <SettingsIcon size={14} className="mr-2 text-muted" /> 设置
            </button>
          </div>
        </div>
      )}
