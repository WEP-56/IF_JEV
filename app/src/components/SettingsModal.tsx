...
interface Props {
  ...
  onTestLlm: (s: Settings) => Promise<string>;
  onTestJev: (s: Settings) => Promise<string>;
  onClearKey: (slot: 'structure' | 'jev') => Promise<void>;
  onClose: () => void;
  onClearData: () => void;
}

export default function SettingsModal({ settings: s, onChange, onTestLlm, onTestJev, onClearKey, onClose, onClearData }: Props) {
          ...
          <div className="flex-1 overflow-y-auto px-7 pb-4">
            {sec === 'llm' && (
              <>
                ...
                <Field label="API Key">
                  <Secret value={s.llm.key} onChange={(v) => setLlm({ key: v })} onClear={() => onClearKey('structure')} />
                </Field>
                <Field label="模型">
                  <Input value={s.llm.model} onChange={(v) => setLlm({ model: v })} mono />
                </Field>
                <Field label="叙事风格">
                ...
                <Field label="单次最大输出">
                  <NumberInput value={s.llm.maxTokens} min={1} max={200000} unit="tokens" onChange={(v) => setLlm({ maxTokens: v })} />
                </Field>
                <Field label="上下文窗口">
                  <NumberInput value={s.llm.context} min={1} max={2000} unit="k tokens" onChange={(v) => setLlm({ context: v })} />
                </Field>
                <TestRow label="真实测试" onTest={() => onTestLlm(s)} />
              </>
            )}
            ...
            {sec === 'jev' && (
              <>
                ...
                <Field label="访问令牌">
                  <Secret value={s.jev.key} onChange={(v) => setJev({ key: v })} onClear={() => onClearKey('jev')} placeholder="留空时使用已保存的密钥" />
                </Field>
                <Field label="模型版本">
                  <Input value={s.jev.model} onChange={(v) => setJev({ model: v })} mono />
                </Field>
                <Field label="时间粒度">
                ...
                <TestRow label="真实判定" onTest={() => onTestJev(s)} />
              </>
            )}
            ...
            {sec === 'about' && (
              <div className="py-5">
                ...
                <p className="mt-5 text-[12.5px] leading-relaxed text-muted">
                  无账号 · 无遥测 · 数据存于本地。设置中的真实测试会调用你配置的服务；故事回合仍为预览流程。
                </p>
              </div>
            )}
          </div>
...
function NumberInput({ value, min, max, unit, onChange }: { value: number; min: number; max: number; unit: string; onChange: (value: number) => void }) {
  return (
    <div className="relative">
      <input
        ...
        className={cn(inputCls, 'pr-20 tabular-nums')}
      />
      <span className="pointer-events-none absolute top-1/2 right-2.5 -translate-y-1/2 text-[11px] text-muted">{unit}</span>
    </div>
  );
}

function Secret({ value, onChange, onClear, placeholder }: { value: string; onChange: (v: string) => void; onClear?: () => void; placeholder?: string }) {
  ...
  return (
    <div className="relative">
      <input
        ...
        className={cn(inputCls, onClear ? 'pr-14 font-mono text-[11.5px]' : 'pr-8 font-mono text-[11.5px]')}
      />
      {onClear && <button type="button" title="清除已保存的密钥" aria-label="清除已保存的密钥" onClick={onClear} className="absolute top-1/2 right-7 -translate-y-1/2 p-1 text-muted hover:text-rose-500"><Trash2 size={13} /></button>}
...
function TestRow({ label, onTest }: { label: string; onTest?: () => Promise<string> }) {
    ...
    <div className="flex items-center gap-3 py-3">
      <button
        ...
        onClick={async () => {
          ...
          setDetail('');
          try {
            if (!onTest) throw new Error('此版本尚未接入生图测试');
            setDetail(await onTest());
            setSt('ok');
          } catch (error) {
      ...
      {(st === 'ok' || st === 'error') && (
        <span className={`fade-up flex min-w-0 items-center gap-1 text-[12.5px] ${st === 'ok' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-500'}`}>
          {st === 'ok' && <Check size={13} className="shrink-0" />}<span className="break-all">{detail}</span>
        </span>
      )}
    </div>
  );
