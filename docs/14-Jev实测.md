...
> 本文是接口事实的依据；[07 Jev 问题库](07-Jev问题库.md) 的接口部分已按本文校正。

本文记录 Jev 后端的黑盒实测结果，用来替换 07 §1 里写于实测之前的接口描述。所有结论来自真实请求，探针脚本见 `tools/jev_probe.py`；本文只描述**实际观察到的行为**，设计决策仍以 `docs/00–13` 为准。
...
- 330 input → `$0.00001386` = 330 × 0.042 / 1e6，**输出 token 未计费**。印证了设计阶段"输入计价、输出免费"的判断。
